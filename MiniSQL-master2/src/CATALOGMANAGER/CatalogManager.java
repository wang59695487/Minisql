...
public class CatalogManager {
	...
	public static void showIndexCatalog() {
		...
		Enumeration<index> en = indexes.elements();
		int cnt=1;
		GUI.StringPrintText("There are "+indexes.size()+" indexes in the database: ");
        GUI.StringPrintText("\tIndex name\tTable name\tAttribute name:");
		while(en.hasMoreElements()) {
			tmpIndex=en.nextElement();			
			GUI.StringPrintText(cnt+++"\t"+tmpIndex.indexName+"\t\t"+tmpIndex.tableName+"\t\t"+tmpIndex.attriName);
		}
	}
	public static void showTableCatalog() {
		...
		Enumeration<table> en = tables.elements();
		int cnt=1;
		GUI.StringPrintText("There are "+tables.size()+" tables in the database: ");
        while(en.hasMoreElements()) {
           tmpTable=en.nextElement();
           GUI.StringPrintText("\nTable "+cnt++);
           GUI.StringPrintText("Table name: "+tmpTable.tableName);
           GUI.StringPrintText("Number of Columns: "+tmpTable.attriNum);
           GUI.StringPrintText("Primary key: "+tmpTable.primaryKey);
           GUI.StringPrintText("Number of tuples: "+tmpTable.tupleNum);
           GUI.StringPrintText("Index keys: "+tmpTable.indexNum);
           GUI.StringPrintText("\tIndex name\tTable name\tAttribute name:");
           for(int i=0;i<tmpTable.indexNum;i++){
        	   tmpIndex=tmpTable.indexes.get(i);
        	   GUI.StringPrintText("\t"+tmpIndex.indexName+"\t"+tmpIndex.tableName+"\t\t"+tmpIndex.attriName);
           }
           GUI.StringPrintText("Attributes: "+tmpTable.attriNum);
           GUI.StringPrintText("\tAttribute name\tType\tlength\tisUnique");
           for(int i=0;i<tmpTable.attriNum;i++){
        	   tmpAttribute=tmpTable.attributes.get(i);
        	   GUI.StringPrintText("\t"+tmpAttribute.attriName+"\t\t"+tmpAttribute.type+"\t"+tmpAttribute.length+"\t"+tmpAttribute.isUnique);
           }
        }      		
	}
	...
	public static boolean isPrimaryKey(String tableName,String attriName){
		if(isTableExist(tableName)){
			...
			GUI.StringPrintText("The table "+tableName+" doesn't exist");
			return false;
		}
	}
	public static boolean inUniqueKey(String tableName,String attriName){
		if(isTableExist(tableName)){
			...
			if(i>=tmpTable.attributes.size()){
				GUI.StringPrintText("The attribute "+attriName+" doesn't exist");
				return false;
			}
		}
		GUI.StringPrintText("The table "+tableName+" doesn't exist");
	...
	public static boolean isIndexKey(String tableName,String attriName){
		if(isTableExist(tableName)){
			...
			if(isAttributeExist(tableName,attriName)){
				...
				//GUI.StringPrintText(" The attribute "+attriName+" is not an index key");留给interpreter
			}
			else{
				GUI.StringPrintText("The attribute "+attriName+" doesn't exist");
			}
	...
	public static int getAttriOffest(String tableName,String attriName){
		...
		GUI.StringPrintText("Error: The attribute "+attriName+" doesn't exist");
		return -1;
	}
	...
	public static String getType(String tableName,int i){//用于insert 针对第i个属性
		table tmpTable=tables.get(tableName);
		//GUI.StringPrintText(tmpTable.attributes.get(i).type+tmpTable.attributes.get(i).attriName);
	...
	public static boolean dropTable(String tableName){
		try{
			...
			GUI.StringPrintText("Error: drop null table. "+e.getMessage());
			return false;
		}
	}
	/*
